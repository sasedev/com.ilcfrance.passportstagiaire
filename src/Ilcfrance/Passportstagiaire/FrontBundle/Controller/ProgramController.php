...
class ProgramController extends IlcfranceController
{
	...
	public function listAction(Request $request)
	{
		...
		$programs = $em->getRepository('IlcfrancePassportstagiaireDataBundle:Program')->getAll();
	...
	public function addPostAction(Request $request)
	{
		...
		if (isset($reqData['ProgramAddForm'])) {
			...
			if ($programAddForm->isValid()) {
				...
				$cacheDriver->delete('Program_getAllQuery');
	...
	public function deleteAction($id, Request $request)
	{
		...
		$em = $this->getEntityManager();
		try {
			$program = $em->getRepository('IlcfrancePassportstagiaireDataBundle:Program')->getOneById($id);
		...
		try {
			...
			if (null == $program) {
				...
				try {
					...
					$response->headers->set('Last-Modified', gmdate('D, d M Y H:i:s', $program->getDtUpdate()->getTimestamp()));
			...
			if (null == $program) {
				...
				if (isset($reqData['ProgramUpdateDescriptionForm'])) {
					...
					if ($programUpdateDescriptionForm->isValid()) {
						...
						$cacheId = 'Program_getOneByIdQuery' . $id;
