...
class UserController extends IlcfranceController
{
	...
	public function listAction(Request $request)
	{
		...
		$users = $em->getRepository('IlcfrancePassportstagiaireDataBundle:User')->getAll();
	...
	public function addPostAction(Request $request)
	{
		...
		if (isset($reqData['UserAddForm'])) {
			...
			if ($userAddForm->isValid()) {
				...
				$cacheDriver->delete('User_getAllQuery');
	...
	public function deleteAction($id, Request $request)
	{
		...
		$em = $this->getEntityManager();
		try {
			$user = $em->getRepository('IlcfrancePassportstagiaireDataBundle:User')->getOneById($id);
		...
		try {
			...
			if (null == $user) {
				...
				if (null == $userPicture) {
					...
					$classname = \str_replace('\\', '.', \strtolower(User::class));

					$cacheId = 'region_User_picture' . $classname . '_' . $id . '__picture';
					$cacheDriver->delete($cacheId);
				} else {
				...
				if (isset($reqData['UserUpdateEmailForm'])) {
					...
					if ($userPictureCropPictureForm->isValid()) {
						try {
							...
							$classname = \str_replace('\\', '.', \strtolower(UserPicture::class));
